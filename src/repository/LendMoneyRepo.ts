...
async function saveLendMoneyDetailsFromBackup(lendMoneyObj: LendMoneyModel) {
    const db = await openDBConnection();
    await db.runAsync('INSERT INTO money_lends (expense_id_fk, paid_amount, timestamp) VALUES (?, ?, ?)',
        lendMoneyObj.expense_id_fk, lendMoneyObj.paid_amount, lendMoneyObj.timestamp
    );
}
...
export {
    ...
    getAllLendMoney,
    saveLendMoneyDetailsFromBackup
}